          ...
          <form @submit.prevent="submitExpense">
            <div class="space-y-3 md:space-y-5">
              ...
              <div class="flex flex-col sm:flex-row sm:items-center">
                ...
                <select v-model="form.category" class="flex-1 sm:w-2/3 border-2 rounded-lg p-2 border-[#D1FAE5] bg-slate-50 focus:outline-none focus:ring-2 focus:ring-[#10B981] focus:border-transparent">
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 md:p-6">
          ...
          <div class="overflow-hidden">
              ...
              <table class="min-w-full">
                ...
                <tbody class="divide-y divide-[#D1FAE5]">
                  <tr v-for="expense in recentExpenses" :key="expense.id" class="border-b-[0.5px] border-b-[#5AE4A8]">
                    ...
                    <td class="py-3 px-2 md:px-4 text-sm font-medium text-center">{{ expense.date }}</td>
            ...
            <div class="mt-4 md:mt-6 flex justify-center">
              <Link :href="route('expenses.all')" class="text-sm font-medium text-[#10B981] hover:text-[#059669] transition duration-200">
          ...
          <table class="min-w-full">
            ...
            <tbody class="divide-y divide-[#D1FAE5]">
              <tr v-for="expense in monthlyExpenses" :key="expense.id" class="border-b-[0.5px] border-b-[#5AE4A8]">
                ...
                <td class="py-2 px-4 text-sm font-medium text-center">{{ expense.date }}</td>
                ...
                <td class="py-2 px-4 text-sm font-medium text-center">
                  <div class="flex justify-center space-x-2">
                    <button @click="editExpense(expense.id)" class="p-1 rounded-md hover:bg-[#D1FAE5] transition-all">
                      <img src="@/Assets/IMG/edit.png" alt="Edit" class="w-5 h-5">
                    </button>
                    <button @click="deleteExpense(expense.id)" class="p-1 rounded-md hover:bg-red-100 transition-all">
          ...
          <div v-for="expense in monthlyExpenses" :key="expense.id" class="bg-white border border-[#D1FAE5] rounded-lg shadow-sm p-4">
            <div class="flex justify-between items-start mb-2">
              <div>
                ...
                <span class="inline-block bg-[#D1FAE5] text-[#10B981] text-xs px-2 py-1 rounded-full mt-1">{{ expense.category.name }}</span>
              </div>
              <span class="font-bold text-[#1E293B]">{{ formatCurrency(expense.amount) }}</span>
            </div>
            <div class="flex justify-between items-center text-sm text-gray-600 mb-3">
              <div>
                <div>Date: <span class="font-medium">{{ expense.date }}</span></div>
            ...
            <div class="flex justify-end space-x-2 pt-2 border-t border-[#D1FAE5]">
              <button @click="editExpense(expense.id)" class="p-2 rounded-full bg-[#D1FAE5] hover:bg-[#A7F3D0] transition-all">
                <img src="@/Assets/IMG/edit.png" alt="Edit" class="w-4 h-4">
              </button>
              <button @click="deleteExpense(expense.id)" class="p-2 rounded-full bg-red-50 hover:bg-red-100 transition-all">
